...
class EditTable extends Component {
  ...
  handlers = {
    ...
    onChangeKey: (value) => {
      ...
      if (this.state.current === 'add' && Object.keys(this.state.userProperty).some(p => p === value)
        ...
        help = formatMessage({id: 'editor.validation.already-property'},
          {id: 'editor.common.key'}
        );
      } else if (!value.length) {
        validateStatus = 'error';
        help = formatMessage(
          {id: 'editor.validation.enter-property'});
      } else {
  ...
  render() {
    ...
    const columns = [
      {
        title: formatMessage({id: 'editor.common.key'}),
        dataIndex: 'key',
      },
      {
        title: formatMessage({id: 'editor.common.value'}),
        dataIndex: 'value',
      },
      {
        title: '',
        ...
        render: (text, record) => {
          return (
            <div>
              <Button className="rde-action-btn" shape="circle" onClick={() => {
    ...
    return (
      <FlexBox flexDirection="column">
        ...
        <Modal
          ...
          visible={visible}
        >
          <Form.Item required label={formatMessage({id: 'editor.common.key'})} colon={false} hasFeedback
                     validateStatus={validateStatus}
                     help={help}>
            <Input defaultValue={tempKey} value={tempKey} onChange={(e) => {
              onChangeKey(e.target.value);
            }}/>
          </Form.Item>
          <Form.Item label={formatMessage({id: 'editor.common.value'})} colon={false}>
            <Input defaultValue={tempValue} value={tempValue} onChange={(e) => {
