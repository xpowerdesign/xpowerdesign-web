...
class IconChooser extends Component {
  ...
  render() {
    ...
    const label = (
      <React.Fragment>
        <span style={{marginRight: 8}}>{formatMessage({id: 'editor.common.icon'})}</span>
    ...
    const title = (
      <div style={{padding: '0 24px'}}>
        <Input
          ...
          placeholder={formatMessage({id: 'editor.imagemap.marker.search-icon'}, {length: filteredIconsLength})}
        />
      </div>
    );
    return (
      <React.Fragment>
        <Form.Item label={label} colon={false}>
          <Button onClick={onClick}>
            {formatMessage({id: 'editor.imagemap.marker.choose-icon'})}
          </Button>
        </Form.Item>
        <Modal
          onOk={onOk}
